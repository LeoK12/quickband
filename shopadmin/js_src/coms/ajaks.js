...
/**
用于处理左侧菜单
如果符合要求,则为元素点击,提交  事件,增加拦截,并进入ShopEx_AdminXHR流程.
**/
      ...
      var isAJAXForm=function(item){
       ...
       $(document.body).addEvents({
              ...
              if (iptFiles.length>0 && iptFiles.some(function(el){
                            ...
                            if(!$('upload_iframe')){
                            new Element('iframe',{'id':'upload_iframe','name':'upload_iframe','styles':{'display':'none'}})
                            ...
                            item.submit();
							item.retrieve('reset_target')?item.target=item.retrieve('reset_target'):item.target='';
							return  item.removeProperties('enctype','encoding');
			  }
                if (f_target) {
                          if('fm' in f_target){
